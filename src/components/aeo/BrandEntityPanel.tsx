...
import { Network, Zap, TrendingUp, AlertCircle, ChevronDown, ChevronUp, ExternalLink, Search } from "lucide-react";
...
type WikidataStatus =
    | { status: "idle" }
    | { status: "loading" }
    | { status: "found"; qid: string; label: string; description: string }
    | { status: "not-found" };
...
export function BrandEntityPanel({ siteId, domain, brandFacts, competitorDomains }: EntityPanelProps) {
    ...
    const [wikidata, setWikidata] = useState<WikidataStatus>({ status: "idle" });
    ...
    async function fetchWikidata() {
        if (wikidata.status !== "idle") return;
        setWikidata({ status: "loading" });
        try {
            const brand = domain.replace(/^www\./, "").split(".")[0];
            const url = `https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(brand)}&language=en&limit=1&format=json&origin=*`;
            const res = await fetch(url);
            const data = await res.json();
            const hit = data.search?.[0];
            if (hit) {
                setWikidata({ status: "found", qid: hit.id, label: hit.label ?? brand, description: hit.description ?? "" });
            } else {
                setWikidata({ status: "not-found" });
            }
        } catch {
            setWikidata({ status: "not-found" });
        }
                ...
                <div className="border-t border-border p-5 space-y-6">
                    ...
                    <div className="pt-2 border-t border-border">
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-1.5">
                            <Search className="w-3.5 h-3.5 text-blue-400" /> Wikidata Entity
                        </p>
                        {wikidata.status === "loading" && (
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin shrink-0" />
                                Checking Wikidata…
                            </div>
                        )}
                        {wikidata.status === "found" && (
                            <div className="flex items-center justify-between px-3 py-2.5 rounded-xl border border-blue-500/20 bg-blue-500/5 text-xs">
                                <div className="min-w-0">
                                    <p className="font-semibold text-blue-300">{wikidata.label} <span className="text-blue-500/60 font-normal ml-1">{wikidata.qid}</span></p>
                                    {wikidata.description && <p className="text-muted-foreground mt-0.5 truncate">{wikidata.description}</p>}
                                </div>
                                <a href={`https://www.wikidata.org/wiki/${wikidata.qid}`} target="_blank" rel="noopener noreferrer" className="ml-3 shrink-0 text-blue-400 hover:text-blue-300 transition-colors">
                                    <ExternalLink className="w-3.5 h-3.5" />
                                </a>
                            </div>
                        )}
                        {wikidata.status === "not-found" && (
                            <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-border bg-muted/20 text-xs text-muted-foreground">
                                <AlertCircle className="w-3 h-3 shrink-0 text-amber-400" />
                                No Wikidata entity found for <span className="font-medium text-foreground ml-1">{domain}</span>
                            </div>
                        )}
                    </div>
