...
function toResult(row: NonNullable<Awaited<ReturnType<typeof prisma.keywordSerpAnalysis.findUnique>>>): ActionResult {
    return {
        data: {
            ...
            yourPageH2s:      (row.yourPageH2s ?? []) as unknown as string[],
            drGap:            row.drGap,
            rdGapRoot:        row.rdGapRoot,
            rdGapPage:        row.rdGapPage,
            clientDR:         row.clientDR ?? 0,
            clientRDs:        row.clientRDs ?? 0,
            pageRDs:          row.rdGapPage ?? 0,
            toxicCount:       row.toxicCount ?? 0,
            topAnchors:       (row.topAnchors ?? []) as unknown as { anchor: string; count: number }[],
            newLastWeek:      row.newLastWeek ?? 0,
            lostLastWeek:     row.lostLastWeek ?? 0,
            dofollowRatio:    row.dofollowRatio ?? 0,
