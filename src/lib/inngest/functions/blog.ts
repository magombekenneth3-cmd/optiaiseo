...
${liveBlogPost.content.substring(0, 80000)}`,
        ...
        if (factCheck.issues.length > 0 || !factCheck.complete) {
            logger.warn(`[Blog/Pipeline] Fact-check status: ${factCheck.coverage}% coverage`, {
                issues: factCheck.issues,
                factCheckAvailable: factCheck.qualityScore !== null,
                checkedChunks: factCheck.checkedChunks,
                totalChunks: factCheck.totalChunks,
            });
        }
        ...
        const contentWithFunnel = await step.run("inject-funnel-cta", async () => {
            try {
                ...
                logger.warn("[Blog/Funnel] inject-funnel-cta threw — using original content", {
                    error: funnelErr instanceof Error ? funnelErr.message : String(funnelErr),
                    siteId,
                    keyword,
                });
                const { sanitizeHtml } = await import("@/lib/sanitize-html");
        ...
        liveBlogPost.content = contentWithFunnel;
        ...
        liveBlogPost.evidenceCoverage = publicationGateStep.evidencePacket.claims.length > 0
            ...
            : publicationGateStep.evidencePacket.availability === "AVAILABLE" ? 100 : 0;
