...
export function useInputControls({
  ...
  // NOTE: We intentionally do NOT use useSessionContext() here.
  // During the session 'connecting' phase, session.local is undefined and
  // destructuring { microphoneTrack } from it crashes the hook — preventing
  // AgentControlBar from rendering and blocking mic permission.
  // Instead, we derive microphoneTrack from microphoneToggle.track below.
  ...
  return {
    // microphoneTrack comes from the toggle, not useSessionContext,
    // to avoid crashes during session 'connecting' state.
    microphoneTrack: microphoneToggle.track as TrackReference | undefined,
    cameraToggle: {
      ...cameraToggle,
